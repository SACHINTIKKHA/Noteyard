                ...
                public function editing($id){
                    ...
                    $path=$file->storeAs('uploads',$filename,'public');
                        ...
                        $del=DB::table('notes')->where('id',$id)->update([
                            ...
                            $sub->validate([
                            'file'=>'required|file|mimes:jpg,png|max:500'
                            ]);
                            if($sub->hasFile('file')){
                                $file=$sub->file('file');
                                $filename=time().'_'.$file->getClientOriginalName();
...
public function allfreefile(){
    $payment=DB::table('users')->where('id',session('id'))->exists();
    if($payment){
        $notes=DB::table('notes')->where('filetype','free')->get();
...
public function allpaidfile(){
    ...
    $payment=DB::table('users')->where('id',session('id'))->exists();
    if($payment){
        $notes=DB::table('notes')->where('filetype','subscription')->get();
